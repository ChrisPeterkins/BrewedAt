...
// Firebase Configuration & Social Media Stats
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        const docRef = db.collection('siteConfig').doc('socialMedia');
        ...
        if (docSnap.exists) {
            ...
            // Update the total followers display
            const totalFollowersElement = document.getElementById('total-followers');
            if (totalFollowersElement) {
                // Format the number with commas
                const formattedTotal = totalFollowers.toLocaleString() + '+';
                totalFollowersElement.textContent = formattedTotal;

                // Optional: Update social media links with actual handles
                const socialLinks = document.querySelectorAll('.social-links-large a');
                if (socialLinks.length >= 3) {
                    if (data.instagram?.handle) {
                        socialLinks[0].href = `https://instagram.com/${data.instagram.handle}`;
                    }
                    if (data.facebook?.handle) {
                        socialLinks[1].href = `https://facebook.com/${data.facebook.handle}`;
                    }
                    if (data.twitter?.handle) {
                        socialLinks[2].href = `https://twitter.com/${data.twitter.handle}`;
                    }
                }
                ...
                if (footerLinks.length >= 3) {
                    if (data.twitter?.handle) {
                        footerLinks[0].href = `https://twitter.com/${data.twitter.handle}`;
                    }
                    if (data.instagram?.handle) {
                        footerLinks[1].href = `https://instagram.com/${data.instagram.handle}`;
                    }
                    if (data.facebook?.handle) {
                        footerLinks[2].href = `https://facebook.com/${data.facebook.handle}`;
                    }
                }
            }
        } else {
            console.log('No social media stats found in database');
            const totalFollowersElement = document.getElementById('total-followers');
            if (totalFollowersElement) {
                totalFollowersElement.textContent = '15,000+';
            }
        }
    } catch (error) {
